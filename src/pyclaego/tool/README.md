...
```
tool/
├── __init__.py          # 导出符号 + 注册 20 种内置工具（导出 21 种工具类）+ get_tool_manager()
...
└── tools/
    ├── __init__.py
    ├── fs_base_tool.py           # 文件系统工具公共基类（FileSystemBaseTool）
    ...
    ├── web_fetch_tool_v2.py      # web_fetch         - 抓取网页内容（带本地缓存）
    └── web_fetch_cache/          # WebFetchToolV2 的本地缓存存储目录
```
...
```python
from pyclaego.llm.types import ImagePart, DocumentPart
...
> `__init__.py` 导出 21 个工具类（含旧版 `WebFetchTool`），但实际注册并可调用的为以下 20 种。
...
| `web_fetch` | `WebFetchToolV2` | True | True | 抓取指定 URL 的网页内容（带本地缓存、截断、版本管理）|
...
# web_fetch 工具（WebFetchToolV2）
web_fetch:
  url: "https://example.com/docs"
  timeout: 30         # 可选，超时秒数
  use_cache: true     # 可选，默认 true；false 时强制重新抓取并检测内容变更
```
...
```python
from pyclaego.tool import ToolCallParser, TOOL_CALL_PROMPT
