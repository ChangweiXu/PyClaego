...
class ConfigManager:
    ...
    # 配置文件搜索路径（按优先级顺序）
    CONFIG_PATHS = [
        Path.home() / ".pyclaego" / "config.yaml",      # ~/.pyclaego/config.yaml
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if not self.config_file:
            print(f"[Config] 未找到配置文件，使用默认配置")
            print(f"[Config] 可在以下位置创建配置文件:")
            for path in self.CONFIG_PATHS:
                print(f"  - {path}")
    ...
    def _resolve_includes(
        ...
        if isinstance(obj, IncludeTag):
            ...
            return {
                key: self._resolve_includes(value, base_dir, include_stack)
    ...
    def _replace_env_vars(self, obj: Any) -> Any:
        ...
        elif isinstance(obj, (IncludeTag, IncludeDirTag)):
            # !include / !include_dir 应在此步骤之前已被 _resolve_includes 消除；
    ...
    def _replace_env_var_in_string(self, text: str) -> Union[str, int, float, bool]:
        """替换字符串中的环境变量
    ...
    def _resolve_config_references(self, obj: Any, visited: Optional[set] = None) -> Any:
        """递归解析配置项引用和拼接
    ...
    def _resolve_config_ref_in_string(self, text: str) -> Union[str, int, float, bool]:
        """解析字符串中的配置项引用
