...
class TaskBridgeClient:
    ...
    def _build_task_tree(self) -> Dict[str, list]:
        """构建任务树结构"""
        result = {}
    ...
    def _build_task_node(self, task: Dict[str, Any], all_tasks: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """递归构建任务节点"""
        node = {
    ...
    async def add_browser_connection(self, websocket: WebSocket) -> None:
        ...
        try:
            await websocket.send_text(json.dumps(initial_message, ensure_ascii=False))
            _rlog.info("web_api", f"[TaskBridgeClient] 已发送初始快照到浏览器，包含 {len(task_tree)} 个 Session")
        except Exception as e:
...
# 全局桥接客户端实例
_bridge_client: Optional[TaskBridgeClient] = None
