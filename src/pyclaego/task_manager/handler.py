...
class SessionTaskHandler:
    ...
    async def __call__(self, msg: Dict[str, Any]) -> Optional["SessionTaskHandler"]:
        raise NotImplementedError("SessionTaskHandler is deprecated. Use SessionTaskHandlerV2 instead.")
    
    async def _handle_task_update(self, task_data: Dict[str, Any]) -> Optional["SessionTaskHandler"]:
    ...
    async def start_subtask(
        ...
        # 检查深度限制
        if self._depth >= 10:
            raise ValueError(f"Task depth exceeds maximum (10)")
...
class SessionTaskHandlerV2(SessionTaskHandler):
    ...
    def get_subagent_id(self) -> Optional[str]:
        """获取子代理 ID"""
