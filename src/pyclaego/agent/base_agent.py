...
class BaseAgent(ABC):
    ...
    def __init__(self, config: Dict[str, Any], session_id: str):
        ...
        """
        self.config = config
        self.agent_type = config.get("type", "unknown")
    ...
    async def process_v2(
        self,
        user_message: Dict[str, Any],
        context_handler: "BaseContextHandlerV3",  # noqa: F821
        session_task_handler: "SessionTaskHandlerV2",  # noqa: F821
        **kwargs
    ) -> str:
    ...
    @staticmethod
    def extract_response_content(response: Union[str, ChatCompletion, Message]) -> str:
        """提取 LLM 响应内容
