...
class SpawnSubagentTool(AgentBaseTool):
    ...
    async def execute(
        ...
        parent_context_snapshot: Optional[Dict[str, Any]] = None,
        ...
        **kwargs,
        ...
        if not subagent_task_handler:
            await session_task_handler.log_error(
                f"[SpawnSubagentTool] 创建 SubAgent 任务失败",
            )
            return self._fail(
                f"创建 SubAgent 任务失败",
                log_tag=f"session_{session_task_handler.get_session_id()}",
    ...
    def _resolve_workspace(self, subagent_id: str) -> Path:
        """从全局 config 推断子 Agent 工作目录路径"""
        config = get_config()
        session_cfg = config.get("session", {})

        # 优先使用 session_workspace_root 中针对 session_id 的自定义路径
        session_workspace_map = session_cfg.get("session_workspace_root", {})
        session_root: Optional[str] = session_workspace_map.get(self.session_id)

        if session_root:
            base = Path(session_root).expanduser()
        else:
            workspace_root = session_cfg.get("workspace_root", "~/.pyclaego/workspaces")
            base = Path(workspace_root).expanduser() / self.session_id

        return base / "subagents" / subagent_id
    ...
    def _build_context(
        ...
        memory_mode: str,
        task_prompt: str,
        parent_context_snapshot: Optional[Dict[str, Any]],
        ...
        config = get_session_config(self.session_id)
        context_config = config.get("context_subagents", {})
        context_config["type"] = subagent_type  # 传递子 Agent 类型，供 ContextHandler 选择或调整行为
        ...
        if subagent_type == "code_explorer":
            ...
            project_root = config.get("session_metadata", {}).get("project_root", ".")
