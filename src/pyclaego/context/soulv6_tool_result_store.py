...
class SoulV6ToolResultStore:
    ...
    _instance: Optional["SoulV6ToolResultStore"] = None

    def __init__(
        self,
        memory_manager: Optional[SoulV6MemoryManager] = None,
    ...
    def render_placeholder(self, artifact: SoulV6StoredArtifact) -> str:
        ...
        lines = [
            ...
            "",
            "---- HEAD (first {0} chars) ----".format(len(artifact.head_text)),
            artifact.head_text,
        ]
        if artifact.tail_text:
            lines.extend([
                "",
                "---- TAIL (last {0} chars) ----".format(len(artifact.tail_text)),
                artifact.tail_text,
            ])
        lines.extend([
            "",
    ...
    async def read(
    ...
    ) -> Optional[SoulV6ReadSlice]:
        """按 char range 读取磁盘上的完整内容

        Args:
            tool_call_id: 工具调用 ID
            char_range: (start, end) 字符索引；None 表示从头读到 max_read_chars
        ...
        if char_range:
            ...
            end = max(start, min(char_range[1], total_chars))
        else:
            start = 0
            end = min(self.max_read_chars, total_chars)

        snippet = content[start:end]
        truncated = False
        if len(snippet) > self.max_read_chars:
            snippet = snippet[: self.max_read_chars]
    ...
    def get_artifact(self, tool_call_id: str) -> Optional[SoulV6StoredArtifact]:
