...
class InfoGathererContextHandler(BaseSubAgentContextHandler):
    ...
    def __init__(
        ...
        # 压缩配置（从 config 中读取，缺省使用默认值）
        strategy_cfg = config.get("info_gatherer", {})
    ...
    async def handle_after_tool_calls(
        ...
        # 调用父类追加消息
        messages = await super().handle_after_tool_calls(truncated_results, last_call_prompt=last_call_prompt)
