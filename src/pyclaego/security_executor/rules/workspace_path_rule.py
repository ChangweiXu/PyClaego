...
_DEFAULT_PUBLIC_PATHS: tuple[str, ...] = ("/tmp", "~/.pyclaego/.cache")
...
class WorkspacePathRule(BaseSecurityRule):
    ...
    def __init__(self, rule_config: Dict[str, Any]) -> None:
        ...
        self._workspace_root = Path(
            session_cfg.get("workspace_root", "~/.pyclaego/workspaces")
        ).expanduser()
    ...
    def _get_project_path(self, session_id: str) -> Optional[Path]:
    ...
    def _expand_placeholders(
        path_str: str, workspace: Path, project: Optional[Path]
    ) -> str:
