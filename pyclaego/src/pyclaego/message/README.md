...
├── tui_client.py               # WebSocket TUI 客户端（主用，生产）
├── tui_gateway.py              # 本地回调 TUI 网关（MVP 版本）
...
```python
from src.message.tui_client import TUIClient
...
### `tui_gateway.py` — 本地回调网关（MVP 版本）

无需 WebSocket，通过回调函数直连调度器，适用于单进程调试场景。

```python
from src.message.tui_gateway import TUIGateway

async def my_callback(message: dict) -> dict:
    return {"content": f"已处理: {message['content']}"}

gateway = TUIGateway(scheduler_callback=my_callback)
await gateway.start()
```

**与 `TUIClient` 的区别**：

| 特性 | `TUIClient` | `TUIGateway` |
|------|-------------|-------------|
| 通信方式 | WebSocket | 函数回调 |
| 是否需要服务器 | 是（`core_server.py`）| 否 |
| Session 支持 | 是 | 否（单轮消息） |
| 消息广播 | 是（多客户端）| 否 |
| 适用场景 | 生产使用 | 本地调试/MVP |
...
```python
from src.message.feishu_client import FeishuClient
...
```python
from src.message.feishu_event_listener import FeishuEventListener
...
```python
from src.message.feishu_gateway import FeishuGateway
from src.message.feishu_client import FeishuClient
