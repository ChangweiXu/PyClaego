...
KIND_STREAM_CONTENT = "stream_content"
...
class TaskArtifactStore:
    ...
    _instance: TaskArtifactStore | None = None
    _lock = asyncio.Lock()

    def __init__(self, cache_root: Path | None = None) -> None:
        if cache_root is None:
            cache_root = Path(PYCLAEGO_DEFAULT_ROOT) / ".cache" / "task_artifact"
        ...
        # 启动时一次性清理过期工件目录
        try:
            logging_cfg = get_config().get("logging", {})
            tas_cfg = logging_cfg.get("task_artifact_store", {})
            ttl_hours = float(tas_cfg.get("artifact_ttl_hours", 0))
            if ttl_hours > 0:
                self._cleanup_expired(ttl_hours)
        except Exception:
            pass  # 清理失败不阻断存储初始化
    ...
    # -- cleanup --
    def _cleanup_expired(self, ttl_hours: float) -> None:
        """一次性清理：删除所有 mtime 超过 ttl_hours 的 task 目录。

        每个 task 对应 cache_root 下的一个子目录；其 mtime 在每次
        attach 写入 index.json 时更新，因此可作为目录活跃度的可靠信号。
        """
        ...
        for entry in self.cache_root.iterdir():
            ...
            try:
                ...
                if mtime < cutoff:
                    ...
                    removed += 1
                    # 清除可能残留的缓存条目
                    self._index_cache.pop(task_id, None)
            except Exception:
                errors += 1
        ...
        if removed or errors:
            _rlog.info(
                "core_service",
                f"[TaskArtifactStore] cleanup done: removed={removed}, errors={errors}, "
    ...
    def fetch(self, task_id: str, artifact_id: str) -> tuple[bytes, str, str] | None:
...
class ArtifactReporter:
    ...
    def __init__(self, task_id: str, store: TaskArtifactStore | None = None) -> None:
    ...
    def llm_response(self, payload: str | dict[str, Any], name: str = "llm_response") -> ArtifactRef:
    ...
    def meta(self, name: str, payload: str | dict[str, Any]) -> ArtifactRef:
        return self._store.attach(self.task_id, KIND_META, payload, name=name)

    def file_edit(self, file_path: str, summary: str | dict[str, Any]) -> ArtifactRef:
        return self._store.attach(
...
def safe_attach(reporter: ArtifactReporter | None, fn_name: str, *args: Any, **kwargs: Any) -> None:
    """安全调用 reporter 的某个方法，捕获所有异常并降级日志，绝不打断主流程。"""
    if reporter is None:
        return
    try:
        getattr(reporter, fn_name)(*args, **kwargs)
    except Exception as e:
        _rlog.debug("core_service", f"[ArtifactReporter] attach ({fn_name}) skipped: {e}")
