...
class TaskType(Enum):
    ...
    MEMORY_READ = "memory_read"           # 记忆读取工具调用
    MEMORY_WRITE = "memory_write"         # 记忆写入工具调用
    # SoulV6 新增
...
class EventType(Enum):
    ...
    新（PersonalSpace 模型）：
    - PS_*       : PersonalSpace 生命周期
    - WIDGET_*   : Widget 生命周期（取代旧的 SESSION_*）
    - TASK_*     : 任务粒度

    旧的 SESSION_* 名称保留为 WIDGET_* 的别名，便于在过渡期内逐步下线
    src/session/ 调用方而不破坏现有订阅者。Cleanup 阶段会移除别名。
    """
    ...
    # Widget 级别事件（取代旧 SESSION_*）
    WIDGET_CREATED = auto()
    WIDGET_STARTED = auto()
    WIDGET_COMPLETED = auto()
    WIDGET_FAILED = auto()
    WIDGET_CANCELLED = auto()
    ...
    # ---- Backward-compat aliases (removed in cleanup phase) ----
    SESSION_CREATED = WIDGET_CREATED
    SESSION_STARTED = WIDGET_STARTED
    SESSION_COMPLETED = WIDGET_COMPLETED
    SESSION_FAILED = WIDGET_FAILED
    SESSION_CANCELLED = WIDGET_CANCELLED
