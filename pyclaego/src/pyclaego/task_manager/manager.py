...
class TaskManager:
    ...
    async def end_child_tasks(
        ...
        for child_task in task.children:
            if child_task.status not in [
                ...
                await termination_method(child_task.task_id, "Parent task termined while subtask still running.")
    ...
    def export_all_tasks(self, include_completed: bool = False) -> dict[str, Any]:
        """导出所有 Session 的任务状态
        ...
        for session_id in self._session_tasks.keys():
            sessions[session_id] = self.export_session_tasks(
                session_id,
    ...
    def build_agent_tree(self, session_id: str) -> list[dict[str, Any]]:
        """从任务树中提取 Agent 层级结构。

        遍历 session 的根任务树，递归搜寻 SUBAGENT_SPAWN 任务类型，
        构建 主 Agent → 子 Agent 的层级树，供前端 AgentTreePanel 直接消费。
        ...
        Returns:
            Agent 节点列表，每个节点结构：
            {
                "id": "req_abc" | "req_abc:sub_xyz",
                "label": "Main Agent" | "code_reviewer · abcd1234",
                "request_id": "req_abc",
                "subagent_id": null | "sub_xyz",
                "task_type": "user_message" | "subagent_spawn",
                "children": [...]
            }
        """
        task_ids = self._session_tasks.get(session_id, [])

        # 获取根任务 (parent_id is None)
        root_tasks = [
        ...
        def _extract_agent(
            ...
            """从单个 Task 及其子树中提取 Agent 节点（返回列表以支持平级穿透）。

            规则：
            - 有 metadata.request_id 且无 subagent_id → 主 Agent 节点
            - task_type == SUBAGENT_SPAWN → 子 Agent 节点（request_id 从父链继承）
            - 其他任务类型 → 穿透（不产生节点，但继续递归子任务寻找 SUBAGENT_SPAWN）

            返回值：总是列表。当前任务是 Agent 节点时返回 [自身+子节点]；
            穿透任务时返回所有子孙 Agent 节点的平级列表。
            """
            meta = task.metadata

            subagent_id: str | None = meta.get("subagent_id")  # type: ignore[assignment]
            subagent_type: str = meta.get("subagent_type", "sub")  # type: ignore[assignment]
            own_request_id: str | None = meta.get("request_id")  # type: ignore[assignment]
            request_id: str | None = own_request_id or parent_request_id

            own_node: dict[str, Any] | None = None

            if subagent_id:
                # SUBAGENT_SPAWN 任务 → 子 Agent 节点
                label = (
                    f"{subagent_type} · {subagent_id[-8:]}"
                    if len(subagent_id) >= 8
                    else f"{subagent_type} · {subagent_id}"
                )
                own_node = {
                    "id": f"{request_id}:{subagent_id}",
                    "label": label,
                ...
                # 有 request_id 的根任务（如 USER_MESSAGE）→ 主 Agent 节点
                own_node = {
                    "id": own_request_id,
                    "label": "Main Agent",
            ...
            # 递归遍历子任务，当前节点的 request_id 向下传递
            child_request_id = (
                request_id if subagent_id
                else (own_request_id or parent_request_id)
            )

            # 收集所有子 Agent 节点（平级列表）
            child_nodes: list[dict[str, Any]] = []
            for child in task.children:
                child_results = _extract_agent(child, child_request_id)
                child_nodes.extend(child_results)

            if own_node is not None:
                # 当前任务是 Agent 节点 → 子节点挂载到自身下
                own_node["children"] = child_nodes
                return [own_node]
            else:
                # 穿透任务 → 提升所有子 Agent 节点为平级列表
                return child_nodes

        agents: list[dict[str, Any]] = []
        for root in root_tasks:
            agents.extend(_extract_agent(root))
