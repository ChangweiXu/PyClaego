...
class SpawnAgent(SimpleAgent):
    ...
    def __init__(self, config: dict[str, Any], session_id: str) -> None:
        ...
        agent_type = self.config.get("type", "spawn")
        strategy_config = self.config.get(agent_type, {})
        ...
        self._spawn_tool: SpawnSubagentTool | None = None
        # 当前 Widget 的工作目录（由 Widget.load() 通过 inject_widget_workspace 注入），
        # 子 Agent 工作目录将在其下建立。
        ...
        **kwargs,
        ...
        try:
            ...
            while round_count <= self.max_tool_rounds:
                ...
                text_reply: str | None = v2_resp.text
    ...
    def _build_spawn_tool(self) -> "SpawnSubagentTool":
        """构建 SpawnSubagentTool 实例（需 _subagent_handler & widget_workspace 已注入）

        Returns:
            SpawnSubagentTool 实例
        """
        ...
        if self._widget_workspace is None:
            raise RuntimeError(
                "widget_workspace 未注入，无法确定子 Agent 工作目录基址"
            )
    ...
    def inject_subagent_handler(self, handler: Callable) -> None:
        ...
        否则等待 inject_widget_workspace 后再构建。
        """
