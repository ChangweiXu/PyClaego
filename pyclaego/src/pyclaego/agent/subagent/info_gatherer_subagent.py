"""InfoGathererSubAgent — 信息收集子 Agent（Agent-Tool-Loop 版）

单次创建，运行完整的 agent-tool-loop：
- 允许工具集：glob / search_text / list_directory / read_file /
              write_file / mkdir / download_file / web_search / web_fetch
- 不允许：bash、read_image_base64 及其他副作用工具
- 图片处理：仅在 RESULT.md 中记录图片路径，由上级 Agent 处理
- 安全：write_file / mkdir / download_file / read_file 受 WorkspacePathRule 保护

配置（来自主 Agent config，按需覆盖）：
  llm:             LLM provider ID（继承主 Agent 配置）
  max_tool_rounds: 最大工具循环轮次（默认 20）
"""
...
from ...llm import ChatResponseV2, ToolCall, ToolCallResult, UnifiedMessage
...
class InfoGathererSubAgent(BaseSubAgent):
    """信息收集子 Agent — 完整 agent-tool-loop，受限工具集"""
    ...
    def __init__(
        ...
        subagent_cfg = config.get("subagents", {}).get("info_gatherer", {})
        self.max_tool_rounds: int = int(subagent_cfg.get("max_tool_rounds", 20))

        _rlog.info(
            f"session_{session_id}",
            f"[InfoGathererSubAgent: {subagent_id}] 初始化完成 "
    ...
    async def process(
        ...
        **kwargs,
        ...
        await subagent_task_handler.log_info(
            f"[InfoGathererSubAgent: {self.subagent_id}] 开始执行. "
        ...
        try:
            ...
            await subagent_task_handler.log_info(
                f"[InfoGathererSubAgent: {self.subagent_id}] 上下文就绪: "
            ...
            while round_count < self.max_tool_rounds:
                ...
                loop_task_handler = await subagent_task_handler.create_subtask(
                    ...
                    name=f"InfoGatherer Loop #{round_count}",
                    metadata={"round": round_count, "agent_type": "info_gatherer"},
                )

                await loop_task_handler.log_info(
                    f"[InfoGathererSubAgent: {self.subagent_id}] === 第 {round_count} 轮工具循环 ==="
                )
                ...
                if not llm_result["success"]:
                    ...
                    await loop_task_handler.log_error(
                        f"[InfoGathererSubAgent: {self.subagent_id}] [round {round_count}] LLM 调用失败: {error_msg}"
                    )
                ...
                tool_calls: list[ToolCall] | None = v2_resp.tool_calls
                ...
                await loop_task_handler.log_info(
                    f"[InfoGathererSubAgent: {self.subagent_id}] [round {round_count}] LLM 回复: "
                ...
                if not tool_calls:
                    ...
                    await loop_task_handler.log_info(
                        f"[InfoGathererSubAgent: {self.subagent_id}] [round {round_count}] 无工具调用，结束循环"
                    )
                    await loop_task_handler.complete()
                ...
                await subagent_task_handler.log_warning(
                    f"[InfoGathererSubAgent: {self.subagent_id}] 达到最大工具调用轮次 ({self.max_tool_rounds})，发起汇总调用"
                )
            ...
            await subagent_task_handler.log_info(
                f"[InfoGathererSubAgent: {self.subagent_id}] 处理完成，共 {round_count} 轮"
            )
            ...
            await subagent_task_handler.log_error(
                f"[InfoGathererSubAgent: {self.subagent_id}] process 异常: {e}\n{traceback.format_exc()}"
            )
            ...
            if cancelled:
                ...
                try:
                    ...
                    await subagent_task_handler.log_error(
                        f"[InfoGathererSubAgent: {self.subagent_id}] handle_after_loop 失败（不影响响应）: "
                        f"{after_e}\n{traceback.format_exc()}"
                    )
        ...
        for tc in tool_calls:
            ...
            for k, v in (tc.arguments or {}).items():
                if len(str(v)) > 50:
                    ...
                    args_preview.append(f"{k}={v!s}")
            args_preview = ", ".join(args_preview)
            await task_handler.log_info(
                f"[InfoGathererSubAgent: {self.subagent_id}] [TOOL] name={tc.name} | round={round_count} | args: {args_preview}"
            )

            # task_handler 会创建新的子任务来执行工具调用，确保日志清晰且可追踪
                ...
                try:
                    ...
                    for k, v in json_content.items():
                        if len(str(v)) > 50:
                            ...
                            result_preview.append(f"{k}={v!s}")
                    result_preview = ", ".join(result_preview)
                except Exception:
                    result_preview = content[:200]
                await task_handler.log_info(
                    f"[InfoGathererSubAgent: {self.subagent_id}] [TOOL] name={tc.name} | success=true | output_len={len(content)} | result=\"{result_preview}\""
                )
            else:
                content = f"[ERROR] {tool_result.get('error', '工具执行失败')}"
                await task_handler.log_error(
                    f"[InfoGathererSubAgent: {self.subagent_id}] [TOOL] name={tc.name} | success=false | error={tool_result.get('error')}"
                )
