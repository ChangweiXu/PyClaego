...
from ..task_manager import ArtifactReporter, SessionTaskHandlerV2, safe_attach
...
def _deserialize_content_part(data: dict):
    return BaseContextHandlerV3.deserialize_content_part(data)
...
class SoulV5ContextHandler(BaseContextHandlerV3):
    ...
    def __init__(
        ...
        # config 是完整 widget_config；self.config 已被基类重绑到 context 切片
        strategy_config = self.config.get("soul_v5", {})
    ...
    async def handle_memory_tool_calls(
        ...
        _SOULV5_WRITE_TOOLS = {
            "soulv5_memory_save_case",
            "soulv5_memory_save_experience",
            "soulv5_memory_update",
            "soulv5_memory_deprecate",
            "soulv5_memory_update_preferences",
        }

        results: list[ToolCallResult] = []
        for tc in memory_calls:
            tool = self._memory_tools[tc.name]
            task_type = TaskType.MEMORY_WRITE if tc.name in _SOULV5_WRITE_TOOLS else TaskType.MEMORY_READ
            sub = await loop_task_handler.create_subtask(
                task_type=task_type,
                name=f"Memory: {tc.name[len(_SOULV5_TOOL_PREFIX):]}",
            ...
            try:
                ...
                RecordStore.get_instance().write_memory_tool_call(
                ...
                safe_attach(reporter, "error_trace", traceback.format_exc(), name=f"error[{tc.name}]")
                RecordStore.get_instance().write_memory_tool_call(
                    session_id=self.session_id,
                    tool_name=tc.name,
                    tool_args=kwargs,
    ...
    async def handle_compress(self) -> str | None:
        """手动压缩入口（/compress 命令）
