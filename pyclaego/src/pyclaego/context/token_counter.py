...
class TokenCounter:
    ...
    def slice_tokens(self, text: str, start: int, end: int) -> str:
        """提取从 start 到 end（不含）token 的文本片段

        Args:
            text: 原始文本
            start: 起始 token 索引（含，0-based）
            end: 结束 token 索引（不含）

        Returns:
            对应 token 范围的文本。若范围越界则自动 clamp。
        """
        if not text:
            return ""
        ...
        try:
            ...
            _rlog.error("core_service", f"[TokenCounter] slice_tokens 失败: {e}，按字符比例截取")
            total_chars = len(text)
            total_est = max(1, total_chars // 3)
            start_char = int(start * total_chars / total_est)
            end_char = int(end * total_chars / total_est)
