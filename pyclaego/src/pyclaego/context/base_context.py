    ...
    def __init__(self, session_id: str, workspace_path: Path, config: dict[str, Any]):
        ...
        Args:
            ...
            config: **完整** widget 配置字典（包含 ``agent`` / ``context`` /
                ``ps_metadata`` / ``context_subagents`` 等顶层键）。
                ``self.config`` 自动绑定到 ``config["context"]`` 切片，便于
                现有子类继续按旧语义读取策略配置；``self.widget_config``
                保留完整字典，供 ``ps_metadata`` 等横切字段访问。
        """
        self.session_id = session_id
        self.workspace_path = workspace_path
        self.widget_config: dict[str, Any] = config or {}
        # 兼容旧语义：self.config = context 配置切片
        self.config: dict[str, Any] = self.widget_config.get("context") or {}
        self.context_type = self.config.get("type", "unknown")
    
    @abstractmethod
    async def get_llm_context(
        self, 
        check_point: ContextCheckPoint | str = ContextCheckPoint.BEFORE_LLM_CALL,
...
class BaseContextHandlerV2(BaseContextHandler):
    ...
    def __init__(
        ...
        # 从 context 切片读取 history 格式（默认 "auto"，自动检测）
        history_format: HistoryFormat = self.config.get("history_format", "auto")  # type: ignore[assignment]
...
class BaseContextHandlerV3(BaseContextHandlerV2):
    ...
    async def get_recent_chat_messages(self, count: int) -> list[dict[str, Any]]:
        """获取最近 count 条用户/助手消息（过滤掉工具调用、工具结果等中间消息）

        直接读磁盘（通过继承自 BaseContextHandlerV2 的 self.history_manager），
        不依赖当前内存状态，可在 process_message 之外安全调用。

        Args:
            count: 要返回的最大消息条数（user + assistant 合计）

        Returns:
            最多 count 条消息（时间升序），每条为原始 dict 格式
        """
        all_msgs = self.history_manager.load_all()
        chat_msgs = [
            m for m in all_msgs
            if m.get("role") in ("user", "assistant")
            and not m.get("tool_calls")
            and not m.get("tool_results")
        ]
        return chat_msgs[-count:] if len(chat_msgs) > count else chat_msgs
    ...
    @staticmethod
    def deserialize_content_part(d: dict):
        """将 content_part dict 反序列化为 ContentPart 对象。

        支持两种格式：
        - 内部格式：{"type": "image", "source_type": "base64"|"url", "data": "...", "media_type": "..."}
        - OpenAI/前端格式：{"type": "image_url", "image_url": {"url": "data:<media>;base64,..." | "https://..."}}
        - 文本：{"type": "text", "text": "..."} 或任何其他 type 值
        """
        from ..llm.types import ImagePart, TextPart
        t = d.get("type")
        if t == "image":
            return ImagePart(
        ...
        if t == "image_url":
            ...
            if url.startswith("data:"):
                ...
                return ImagePart(source_type="base64", data=data, media_type=media_type or "image/png")
            return ImagePart(source_type="url", data=url)
