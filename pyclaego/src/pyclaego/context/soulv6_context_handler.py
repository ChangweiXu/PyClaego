...
from .soulv6_stale_evictor import SoulV6EvictAction, SoulV6StaleEvictor
from .soulv6_tool_result_store import SoulV6ToolResultStore
from .soulv6_turn_brief import SoulV6TurnBrief, SoulV6TurnBriefSynthesizer
from .soulv6_write_review import (
    SoulV6MemoryWriteReview,
    SoulV6WriteAction,
)
...
class SoulV6ContextHandler(SoulV5ContextHandler):
    ...
    def __init__(
        ...
        # config 是完整 widget_config；self.config 已被基类绑定到 context 切片
        strategy_config: dict[str, Any] = self.config.get("soul_v6", {})
        ...
        self._last_budget_plan: SoulV6BudgetPlan | None = None
    ...
    def allocate_budget(self, total_window: int | None = None) -> SoulV6BudgetPlan:
    ...
    async def _build_tool_list(self) -> list[ToolDefinition] | None:  # type: ignore[override]
    ...
    async def handle_after_tool_calls(  # type: ignore[override]
        ...
        for tr in tool_results:
            ...
            if tr.tool_name == _SOULV6_TOOL_RESULT_READ:
                processed_results.append(tr)
                continue
    ...
    async def get_system_prompt(self, user_text: str = "") -> str | None:  # type: ignore[override]
