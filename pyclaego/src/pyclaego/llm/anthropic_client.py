...
class AnthropicClient(LLMClient):
    ...
    async def chat_completion_v2(
        ...
        **kwargs
        ...
        # 6. 透传额外参数（per-call 优先），过滤上游泄漏的非 API 参数
        ...
        try:
            ...
            error_msg = f"Anthropic API 调用失败 (v2): {e!s}"
    ...
    async def chat_completion_stream(
        ...
        **kwargs
        ...
        """流式 LLM 调用接口（Anthropic 实现）。

        使用 ``messages.stream()`` 发起 Anthropic 流式请求，
        逐事件解析 content_block_start/delta/stop 并 yield 统一 StreamChunk。
        ...
        # tool_use_id → {name, input_json_fragments}
        ...
        try:
            async with self.client.messages.stream(**api_params) as stream:
                async for event in stream:
                    ...
                    # ── content_block_delta ────────────────────────
                    elif hasattr(event, "delta") and event.type == "content_block_delta":
                        delta = event.delta
                        if hasattr(delta, "type"):
                            if delta.type == "text_delta":
                                ...
                                # Anthropic 流式工具参数通过 input_json_delta 传递
                                # 需要找到当前活跃的 tool_use block
                                if tool_calls_acc:
                                    last_tc_id = next(reversed(tool_calls_acc))
                                    tool_calls_acc[last_tc_id]["input_json"].append(
                                        delta.partial_json
                                    )
                                    yield StreamChunk(
                                ...
                                # 累计 thinking 增量文本到最后一个 thinking 块
                                thinking_text = getattr(delta, "thinking", "")
                                if thinking_text:
                                    if thinking_blocks and thinking_blocks[-1].get("type") in ("thinking", "redacted_thinking"):
                                        thinking_blocks[-1]["thinking"] = thinking_blocks[-1].get("thinking", "") + thinking_text
                                    else:
                                        thinking_blocks.append({"type": "thinking", "thinking": thinking_text, "signature": ""})
                                    yield StreamChunk(
                                        type="thinking_delta",
                                        thinking_delta=thinking_text,
                                    )
                            elif delta.type == "signature_delta":
                                # 更新最后一个 thinking block 的 signature
                                if thinking_blocks:
                                    thinking_blocks[-1]["signature"] = delta.signature

                    # ── content_block_stop ─────────────────────────
                    elif hasattr(event, "content_block") and event.type == "content_block_stop":
                        pass  # 在流结束后统一发送 tool_call_end

                    # ── message_delta（stop_reason + usage）─────────
                    elif hasattr(event, "delta") and event.type == "message_delta":
                        msg_delta = event.delta
                        if hasattr(msg_delta, "stop_reason"):
                            anth_stop = msg_delta.stop_reason
                            if anth_stop == "tool_use":
            ...
            error_msg = f"Anthropic API 流式调用失败: {e!s}"
            _rlog.error("core_service", f"[AnthropicClient] {error_msg}")
            raise RuntimeError(error_msg)

        # ── 流结束后：发送完整的 tool_call_end ────────────────────
        for tc_id in tool_calls_acc:
            tc = tool_calls_acc[tc_id]
            input_str = "".join(tc["input_json"])
            try:
                ...
                _rlog.warning(
                    "core_service",
                    f"[AnthropicClient] 流式工具参数 JSON 解析失败: {input_str!r}",
                )
            parsed_tc = ToolCall(id=tc_id, name=tc["name"], arguments=args)
        ...
        reasoning: ReasoningArtifact | None = (
            AnthropicThinkingBlocks(blocks=thinking_blocks)
            if thinking_blocks
            else None
        )
