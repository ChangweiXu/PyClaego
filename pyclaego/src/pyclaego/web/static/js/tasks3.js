...
class TaskDashboardV3 {
    ...
    _renderSessions() {
        ...
        for (const sid of sessions) {
            ...
            // v2: session_id == belonging.key() ("ps__widget" or "ps__widget__subagent")
            const parts = sid.split('__');
            const psId = parts[0] || sid;
            const widgetId = parts[1] || '';
            const subagentId = parts[2] || '';
            const chips = [
                `<span class="chip chip-ps">PS ${this._esc(psId)}</span>`,
                widgetId ? `<span class="chip chip-widget">${this._esc(widgetId)}</span>` : '',
                subagentId ? `<span class="chip chip-sa">sa:${this._esc(subagentId)}</span>` : '',
            ].filter(Boolean).join(' ');
            li.innerHTML = `
                <div class="session-id">${chips}</div>
