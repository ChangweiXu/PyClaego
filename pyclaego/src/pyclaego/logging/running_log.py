...
使用示例：
    ...
    rlog.error("session_abc", "工具调用失败: bash")
"""
...
class RunningLog:
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            log_root_str = logging_cfg.get("log_root", PYCLAEGO_DEFAULT_LOGS_ROOT)
            ...
            self._log_root = Path(PYCLAEGO_DEFAULT_LOGS_ROOT).expanduser().resolve()
    ...
    def exception(self, name: str, message: str) -> None:
        """写入 ERROR 级别日志并附加当前异常的 traceback。

        镜像 ``logging.Logger.exception``：在 ``except`` 块内调用时，
        自动捕获活跃异常的完整 traceback 并追加到 message 末尾。
        在 ``except`` 块外调用时，行为与 ``error()`` 相同。
        ...
        self.log(name, full, "ERROR")

    def get_log_file_path(self, name: str, dt: datetime | None = None) -> Path:
...
# 模块级单例（延迟初始化）
_running_log: RunningLog | None = None
