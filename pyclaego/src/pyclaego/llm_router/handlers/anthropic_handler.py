"""Anthropic-protocol handlers (POST /v1/messages, unary + SSE stream)."""
...
from ..recording.stream_merger import merge_anthropic_stream
    ...
    async def handle(request: Request, path: str) -> Response:
        ...
        try:
            ...
            try:
                ...
                resp_body = resp.text
            usage = usage_extract.extract_anthropic_unary(resp_body)
            if status >= 400:
                _rlog.warning("llm_router", f"[anthropic] {route.alias} upstream {status}: {resp_body}")
        except Exception as exc:
            error = type(exc).__name__
            _rlog.exception("llm_router", f"[anthropic] unary forward failed for {route.alias}: {exc}")
...
class AnthropicStreamHandler:
    ...
    async def handle(request: Request, path: str) -> Response:
        ...
        if resp.status_code >= 400:
            ...
            resp_headers = dict(resp.headers)
            finished = now()
            _rlog.warning("llm_router", f"[anthropic] {route.alias} upstream {resp.status_code}: {err_body}")
            await record_call(
                ctx=ctx, route=route, method="POST",
                url=str(route.upstream.base_url) + path,
                request_headers=headers, request_params=None,
        ...
        async def gen() -> AsyncIterator[bytes]:
            ...
            try:
                ...
                error = type(exc).__name__
                _rlog.error("llm_router", f"[anthropic] {route.alias} stream failed: {exc}")
                yield (f'data: {{"error":"{exc}"}}\n\n').encode()
            finally:
                await resp.aclose()
                finished = now()
                usage = usage_extract.extract_anthropic_stream(captured)
                if status is not None and status >= 400:
                    _rlog.warning(
                        "llm_router",
                        f"[anthropic] {route.alias} upstream stream {status}: {''.join(captured)[:500]}",
                    )
                import asyncio as _aio
                merged = merge_anthropic_stream(captured) if not error else None
                _aio.create_task(
                    record_call(
                        ctx=ctx, route=route, method="POST",
                        url=str(route.upstream.base_url) + path,
